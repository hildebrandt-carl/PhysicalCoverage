...
truth_data_path = '../../PhysicalCoverageData/' + str(args.scenario) +'/random_tests/processed/' + str(args.ground_truth_samples) + "/"
...
# Get the file names
original_data_path = '../../PhysicalCoverageData/' + str(args.scenario) +'/random_tests/processed/' + str(args.total_samples) + "/"
...
for i in range(len(beam_numbers)):
    ...
    for j in range(len(crash_file)):
        ...
        if not np.isnan(data):
            ...
            if data not in truth_unique_crashes:
                total_possible_crashes += 1
                truth_unique_crashes.add(data)
...
plt.axvline(x=switch_point_x, color='red', linestyle='--')
...
plt.yticks(np.arange(0, 100 + 1e-6, 5))
plt.legend()
plt.grid()

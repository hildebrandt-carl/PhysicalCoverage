...
print("Checking for Highway-Env Data")

load_name = ""
load_name += "_s" + str(hw_steering_angle) 
load_name += "_b" + str(args.RRS_number) 
load_name += "_d" + str(hw_max_distance) 
load_name += "_t" + str(args.highway_number_tests)
load_name += ".npy"

# Get the file names
hw_base_path                = '{}/{}/random_tests/physical_coverage/processed/{}/{}/'.format(args.data_path, "highway", args.distribution, args.highway_number_tests)
hw_trace_file_names         = glob.glob(hw_base_path + "traces_*" + load_name)

# Get the feasible vectors
hw_base_path = '{}/{}/feasibility/processed/{}/'.format(args.data_path, "highway", args.distribution)
hw_feasible_file_names = glob.glob(hw_base_path + "*_b{}.npy".format(args.RRS_number))

# Get the RRS numbers
hw_trace_RRS_numbers = get_beam_number_from_file(hw_trace_file_names)
hw_feasibility_RRS_numbers = get_beam_number_from_file(hw_feasible_file_names)

print("Checking for BeamNG Data")

load_name = ""
load_name += "_s" + str(ng_steering_angle) 
load_name += "_b" + str(args.RRS_number) 
load_name += "_d" + str(ng_max_distance) 
load_name += "_t" + str(args.beamng_number_tests)
load_name += ".npy"

# Get the file names
ng_base_path                = '{}/{}/random_tests/physical_coverage/processed/{}/{}/'.format(args.data_path, "beamng", args.distribution, args.beamng_number_tests)
ng_trace_file_names         = glob.glob(ng_base_path + "traces_*" + load_name)

# Get the feasible vectors
ng_base_path = '{}/{}/feasibility/processed/{}/'.format(args.data_path, "beamng", args.distribution)
ng_feasible_file_names = glob.glob(ng_base_path + "*_b{}.npy".format(args.RRS_number))

# Get the RRS numbers
ng_trace_RRS_numbers = get_beam_number_from_file(ng_trace_file_names)
ng_feasibility_RRS_numbers = get_beam_number_from_file(ng_feasible_file_names)
...
# Get the RRS numbers
wm_trace_RRS_numbers = get_beam_number_from_file(hw_trace_file_names)
wm_feasibility_RRS_numbers = get_beam_number_from_file(hw_feasible_file_names)


print("Combining data")
# Find the set of beam numbers which all sets of files have
RRS_numbers = list(set(hw_trace_RRS_numbers) | set(hw_feasibility_RRS_numbers) | set(ng_trace_RRS_numbers) | set(ng_feasibility_RRS_numbers) | set(wm_trace_RRS_numbers) | set(wm_feasibility_RRS_numbers))
...
for i in range(len(RRS_numbers)):
    ...
    hw_trace_file       = hw_trace_file_names[i]
    hw_feasibility_file = hw_feasible_file_names[i]

    ng_trace_file       = ng_trace_file_names[i]
    ng_feasibility_file = ng_feasible_file_names[i]

    wm_trace_file       = wm_trace_file_names[i]
    wm_feasibility_file = wm_feasible_file_names[i]

    # Skip if any of the files are blank
    if hw_trace_file == "" or hw_feasibility_file == "" or ng_trace_file == "" or ng_feasibility_file == "" or wm_trace_file == "" or wm_feasibility_file == "":
        print("Could not find one of the files")
        continue

    # Create the list of scenarios and files        
    scenarios           = ["Highway", "BeamNG", "Waymo"]
    trace_files         = [hw_trace_file, ng_trace_file, wm_trace_file]
    feasibility_files   = [hw_feasibility_file, ng_feasibility_file, wm_feasibility_file]

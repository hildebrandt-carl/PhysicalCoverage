...
for key in results:
    ...
    # Convert the unique vectors to sets
    original_v = set()
    for v in o_unique_vectors:
        original_v.add(tuple(v))

    unseen_v = set()
    for v in u_unique_vectors:
        unseen_v.add(tuple(v))

    feasible_vectors = np.array(feasible_vectors)
    feasible_vectors = feasible_vectors[np.lexsort(np.transpose(feasible_vectors)[::-1])]
    feasible_v = set()
    for v in feasible_vectors:
        feasible_v.add(tuple(v))

    feasible_vectors_str = np.array([str(l).strip("[]") for l in feasible_vectors])

    # Get the difference between the original coverage, and the new coverage
    never_seen_set = feasible_v - original_v - unseen_v

    ns = []
    for i in never_seen_set:
        ns.append(i)
    ...
    fig = plt.figure(total_beams)
    plt.bar(feasible_vectors_str, seen_count)
    plt.xticks(feasible_vectors_str, feasible_vectors_str, rotation='vertical')
    plt.xlabel("Feasible Vectors")
    plt.ylabel("Not seen")
